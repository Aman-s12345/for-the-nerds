...
func (a *array) MaxSlidingWindowWithDeque(nums []int, k int) []int {
    deque := []int{}
    ans := []int{}

    for i:=0 ; i<len(nums) ; i++{

        // remove index which are not part of current window
        if len(deque) > 0 && deque[0] <= i-k {
			deque = deque[1:]
		}
        // sorting the deque // means maintainig deque[0] > deque[1] > deque[2]
